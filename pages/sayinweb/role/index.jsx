...
export default function Role() {
  ...
  useEffect(() => {
    fetchRoles();
  }, [pagination.page, pagination.perpage, search, state.sortItems["Role"]]);

  useEffect(() => {
    const socket = getSocket();
    socket.on("Role:create", fetchRoles);
    socket.on("Role:update", fetchRoles);
    socket.on("Role:delete", fetchRoles);
  }, []);

  const fetchRoles = async () => {
    setRoles([]);
    dispatch({ type: "SET_STATE", payload: { loading: true } });
    const [err, response] = await http.get("/sayin/roles", {
      ...pagination,
      search,
      fromdate,
      todate,
      sort: state.sortItems["Role"]
